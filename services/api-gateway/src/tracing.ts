...
process.env.OTEL_RESOURCE_ATTRIBUTES  = process.env.OTEL_RESOURCE_ATTRIBUTES  ?? 'deployment.environment=development'
...
try { sdk.start() } catch { /* OTEL init failure is non-fatal */ }
process.on('SIGTERM', () => { try { sdk.shutdown() } catch {} })
